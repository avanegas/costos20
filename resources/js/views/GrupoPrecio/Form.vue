	...
	export default {
		data() {
			return {
				...
				isProcessing: false,
				initializeURL: `/api/grupo_precios/create`,
				storeURL: `/api/grupo_precios`,
				action: 'Create'
			}
		},
		created() {
			if(this.$route.meta.mode === 'edit') {
				this.initializeURL = `/api/grupo_precios/${this.$route.params.id}/edit`
				this.storeURL = `/api/grupo_precios/${this.$route.params.id}?_method=PUT`
				this.action = 'Update'
			}
		...
		methods: {
				...
				post(this.storeURL, form)
				    .then((res) => {
				        if(res.data.saved) {
				            ...
				            this.$router.push(`/grupo_precios`)
				        }
			...
			remove() {
				del(`/api/grupo_precios/${this.$route.params.id}`).then((res) => {
					Flash.setSuccess('Ha eliminado correctamente el Grupo de precios!')
					this.$router.back()
				});
      		}
		}
