	...
	export default {
		data() {
			return {
				...
				isProcessing: false,
				initializeURL: `/api/grupo_obreros/create`,
				storeURL: `/api/grupo_obreros`,
				action: 'Create'
			}
		},
		created() {
			if(this.$route.meta.mode === 'edit') {
				this.initializeURL = `/api/grupo_obreros/${this.$route.params.id}/edit`
				this.storeURL = `/api/grupo_obreros/${this.$route.params.id}?_method=PUT`
				this.action = 'Update'
			}
		...
		methods: {
				...
				post(this.storeURL, form)
				    .then((res) => {
				        if(res.data.saved) {
				            ...
				            this.$router.push(`/grupo_obreros`)
				        }
			...
			remove() {
				del(`/api/grupo_obreros/${this.$route.params.id}`).then((res) => {
					Flash.setSuccess('Ha eliminado correctamente el Grupo de obreros!')
					this.$router.back()
				});
      		}
		}
